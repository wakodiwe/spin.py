...
test: # Syntax-check spin.py
	@PYTHONPYCACHEPREFIX= python -m py_compile $(SCRIPT) && echo "syntax: OK"

install: # Copy spin.py to ~/.local/lib/py/spin.py
...
devinstall: # Symlink spin.py for live development
	@mkdir -p $${HOME}/.local/lib/py
...
format: # Format spin.py with black (if installed)
